...
export default function GelirlerPage() {
  const [finances, setFinances] = useState({ incomes: [] });
  ...
  const fetchFinances = async () => {
    try {
      ...
      setFinances({ incomes: data.incomes });
      setLoading(false);
    } catch (error) {
  ...
  const deleteItem = async (id) => {
    try {
      const response = await fetch(`/api/finances?id=${id}&type=income`, {
        method: "DELETE",
      });
  ...
  const getFilteredData = () => {
    ...
    if (dateFilter === "1week") {
      ...
      return finances.incomes.filter((item) => {
    ...
    return finances.incomes.filter((item) => new Date(item.date) >= startDate);
  };
  ...
  const filteredIncomes = getFilteredData();
          ...
          <div className="space-y-3">
            {filteredIncomes.length === 0 ? (
              ...
              filteredIncomes.map((income) => (
                <div
                  key={income.id}
                  className="flex items-center justify-between p-4 border-2 border-gray-200 rounded-xl hover:border-green-300 hover:bg-green-50 transition-all"
                >
